...
class AC_GAN(GAN):
    def __init__(self, ce_weight=1, classify_generated=False, **kwargs):
    ...
    def additional_discriminator_losses(self):
        losses = []
        cls_real = self.ce_weight * ktf.reduce_mean(ktf.nn.sparse_softmax_cross_entropy_with_logits
        ...
        if self.classify_generated:
            cls_fake = self.ce_weight * ktf.reduce_mean(ktf.nn.sparse_softmax_cross_entropy_with_logits
